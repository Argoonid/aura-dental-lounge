...
export const IntroAnimation: React.FC<Props> = ({ onComplete }) => {
  ...
  // Хронометраж проката лазера — ровно 1.4 секунды
  ...
  useEffect(() => {
    const handleFrame = (now: number) => {
      ...
      if (p < 1) {
        ...
        setPhase('complete');
        // Фиксация выгравированного логотипа перед бесшовным растворением
        setTimeout(() => setPhase('dissolving'), 450);
        setTimeout(() => {
          setPhase('done');
          onComplete();
        }, 1050);
  ...
  // Траектория луча: от 240px до 760px
  const laserX = 240 + (520 * (progress / 100));

  // Микроповорот лимба нониуса
  const reticleAngle = (progress / 100) * 24;

  return (
    <div 
      onClick={skip}
      className={`fixed inset-0 z-[100] bg-[#FAF7F2] text-[#1E1D1B] flex items-center justify-center p-6 select-none cursor-pointer overflow-hidden transition-all duration-700 ease-out font-sans ${
        phase === 'dissolving' 
          ? 'opacity-0 scale-[1.01] blur-sm pointer-events-none' 
          : 'opacity-100 scale-100 blur-0'
      }`}
    >
      {/* ЦЕНТРАЛЬНАЯ АРХИТЕКТУРНАЯ СЦЕНА В МЕРТВОМ ЦЕНТРЕ ЭКРАНА */}
      <div className="relative w-full max-w-[900px] aspect-[16/9] flex items-center justify-center">
        ...
        >
          <defs>
            {/* Маска проявления: всё левее лазера проявляется из чистого пространства */}
            <clipPath id="laserRevealWindow">
              <rect x="0" y="0" width={laserX} height="520" />
            </clipPath>

            {/* Мягкий след остывания */}
          ...
          {/* 1. ОПТИЧЕСКИЙ ЛИМБ CARL ZEISS (R = 230) */}
          ...
          {/* 2. ЛОГОТИП AURA — ПРОЯВЛЕНИЕ ИЗ ПУСТОТЫ */}
          ...
          {/* 3. ЛАЗЕРНЫЙ СРЕЗ ВЫСОКОГО КОНТРАСТА */}
          {progress < 99.5 && (
            <g id="laser-beam">
              <rect
                x={laserX - 50}
